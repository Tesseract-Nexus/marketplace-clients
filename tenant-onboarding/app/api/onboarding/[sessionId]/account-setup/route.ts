...
export async function POST(
  ...
  try {
    ...
    if (body.auth_method !== 'google') {
      if (!body.password || body.password.length < 10) {
        return errorResponse('Password must be at least 10 characters', 400);
      }
      if (!/[A-Z]/.test(body.password) || !/[a-z]/.test(body.password) || !/[0-9]/.test(body.password) || !/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?`~]/.test(body.password)) {
        return errorResponse('Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character', 400);
      }
    }
