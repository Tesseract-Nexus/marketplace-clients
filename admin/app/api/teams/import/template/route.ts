...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${STAFF_SERVICE_URL}/teams/import/template?format=${format}`,
      { headers }
    );
    ...
    if (format === 'csv' || format === 'xlsx') {
      ...
      responseHeaders.set('Content-Disposition', response.headers.get('Content-Disposition') || `attachment; filename=team_import_template.${format}`);
    ...
    return handleApiError(error, 'GET teams/import/template');
  }
}
