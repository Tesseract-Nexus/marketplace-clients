...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === 'csv' || format === 'xlsx') {
      ...
      headers.set('Content-Disposition', response.headers.get('Content-Disposition') || `attachment; filename=team_import_template.${format}`);
    ...
    console.error('Error fetching team import template:', error);
    return NextResponse.json(
