...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${INVENTORY_SERVICE_URL}/warehouses/import/template?format=${format}`,
      {
        headers: {
    ...
    if (format === 'csv' || format === 'xlsx') {
      ...
      headers.set('Content-Disposition', response.headers.get('Content-Disposition') || `attachment; filename=warehouses_import_template.${format}`);
