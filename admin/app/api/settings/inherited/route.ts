...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${SETTINGS_SERVICE_BASE}/api/v1/settings/inherited${queryString ? `?${queryString}` : ''}`;
    ...
    console.error('Error fetching inherited settings:', error);
    return NextResponse.json(
      { success: false, message: 'Failed to fetch inherited settings' },
      { status: 500 }
    );
  }
}
