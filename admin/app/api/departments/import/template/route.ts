...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === 'csv' || format === 'xlsx') {
      ...
      responseHeaders.set('Content-Disposition', response.headers.get('Content-Disposition') || `attachment; filename=department_import_template.${format}`);
    ...
    return handleApiError(error, 'GET departments/import/template');
  }
}
