...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${STAFF_SERVICE_URL}/staff/import/template?format=${format}`,
      { headers }
    );
    ...
    if (format === 'csv' || format === 'xlsx') {
      ...
      responseHeaders.set('Content-Disposition', response.headers.get('Content-Disposition') || `attachment; filename=staff_import_template.${format}`);
    ...
    return handleApiError(error, 'GET staff/import/template');
  }
}
