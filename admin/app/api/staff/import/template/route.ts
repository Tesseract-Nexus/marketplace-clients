...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === 'csv' || format === 'xlsx') {
      ...
      headers.set('Content-Disposition', response.headers.get('Content-Disposition') || `attachment; filename=staff_import_template.${format}`);
    ...
    console.error('Error fetching staff import template:', error);
    return NextResponse.json(
